...
class BokehDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        LR_size = 384
        rnd_h = random.randint(0, max(0, 1440 - LR_size))
        rnd_w = random.randint(0, max(0, 1920 - LR_size))
        (left, upper, right, lower) = (rnd_w, rnd_h, rnd_w + LR_size, rnd_h + LR_size)
        source = source.crop((left, upper, right, lower))
        target = target.crop((left, upper, right, lower))
        alpha =  alpha.crop((left, upper, right, lower))
        ...
        if self._transform:
            ...
            #src_lens = self._transform(src_lens)
            #tgt_lens = self._transform(tgt_lens)
            #disparity = self._transform(disparity)
